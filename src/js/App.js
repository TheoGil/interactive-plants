...
class App {
  ...
  initColorPalette() {
    this.palette =
      niceColorPalettes[Math.floor(Math.random() * niceColorPalettes.length)];
  }

  initPlant() {
    const garnementsStructure = ["symmetrical", "alternate", "random"][
      Math.floor(Math.random() * 3)
    ];
    const garnementsType = ["leaves", "berries", "flowers"];
    const garnementsTypes = pickRandomElementsFromArray(
      garnementsType,
      Math.ceil(Math.random() * garnementsType.length)
    );
